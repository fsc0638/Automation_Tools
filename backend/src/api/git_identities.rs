...
async fn create_identity(
    ...
    // Strict User KEK gate: Git credentials are user secrets. Normal API paths
    // must never silently fall back to the System KEK; that key is reserved for
    // explicit admin recovery tooling.
    let user_kek = state.session_keys.get_cipher(auth_user.id).ok_or_else(|| {
        AppError::Unauthorized(
            "User KEK session expired — please log in again before saving git credentials".into(),
        )
    })?;
    let encrypted_token = user_kek

...
export function FileWorkspace({
  ...
  onChange: () => void;
  /** False ⇒ hide upload + delete; the list is still rendered so the
   *  non-participant can read filenames. */
  canEdit: boolean;
}) {
  const t = useT();
  ...
  return (
    <div className="rounded-2xl border border-[#E2E8F0] bg-white p-5">
      ...
      {attachments.length === 0 && canEdit ? (
        ...
        <div className="rounded-xl border border-dashed border-[#CBD5E1] bg-[#F8FAFC] px-3 py-8 text-center text-[12px] text-[#94A3B8]">
          目前沒有附件
        </div>
      ) : (
